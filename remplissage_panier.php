    ...
    while ($data = mysqli_fetch_assoc($resultat2))
      {
            if($ID == $data['ID_article'])
            {
                $prix_panier=$data['Prix'];
                $quantite_panier=$data['Quantite'];
...
{
	$sql = "UPDATE panier SET prix = $prix_panier+($quantite * $prix), Quantite = $quantite_panier+$quantite WHERE panier.ID_numero_article = $ID_panier";
...
else
{
	$prix_final=$prix*$quantite;
	    $sql = "INSERT INTO panier (ID_numero_article,ID_article,Prix,Quantite) VALUES (NULL,'$ID', '$prix_final','$quantite')";
